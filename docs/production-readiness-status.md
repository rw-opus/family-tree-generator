...
The versioned-payload tranche is implemented in stacked
[pull request 59](https://github.com/rw-opus/family-tree-generator/pull/59).
Its required Quality, E2E and Security checks have also passed, including the
database-trigger, direct PostgREST, rollback and entitlement regressions. It
depends on pull request 57 and remains undeployed pending independent review.
...
| Tree schema/version and payload validation | Verified in required CI; deployment pending | PR 59 adds a persisted serialization version, strict client validation, local/cloud quarantine, GEDCOM limits and a database trigger for every direct write; its fresh-stack Security workflow passed direct API, rollback and entitlement regressions            | Obtain independent approval, then deploy through staging after a production aggregate-only preflight                              |

...
const familyTreeUpdateRecord = (tree) => ({
  title: tree.title || "Untitled family tree",
  people: tree.people || [],
  tree_data: tree,
});
...
export async function saveFamilyTree(tree, ownerId = "") {
  let query = supabase.from("family_trees").update(familyTreeUpdateRecord(tree)).eq("id", tree.id);
  if (ownerId) query = query.eq("owner_id", ownerId);

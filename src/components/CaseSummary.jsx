...
export function CaseSummary({ tree }) {
  ...
  return <section className="case-summary" aria-label="Case summary">
    ...
    <div className="summary-heirs"><h3><UsersRound size={17} /> Heirs and published causa mortis values</h3>{heirs.length ? heirs.map((heir) => { const item = coverageFor(heir.id); const fraction = approximateFraction(item?.publishedFraction || 0); return <div className="summary-heir-row" key={heir.id}><span><strong>{heir.name || "Unnamed heir"}</strong><small>{Number(heir.sharePercent || 0).toLocaleString("en-MT", { maximumFractionDigits: 6 })}% succession share</small></span><span><strong>{item?.publishedCount ? `${fraction.numerator}/${fraction.denominator}` : "No published DCM"}</strong><small>{item?.publishedCount ? money.format(item.publishedValue) : "No declared value recorded"}</small></span></div>; }) : <p className="helper-text">No effective heirs have been entered.</p>}</div>
  </section>;
}

import { Building2, Calculator, CalendarDays, FileText, MapPin, UsersRound } from "lucide-react";
...
export function CaseSummary({ tree }) {
  ...
  const propertyTaxReports = (tree.properties || []).map((property) => ({
    property,
    report: buildPropertyVendorTaxReport(property, tree.people || [], tree.outsideParties || []),
  }));
  ...
  return (
    <section className="case-summary" aria-label="Case summary">
      <header>
        ...
        <h2>{tree.title || "Unnamed family tree"}</h2>
      </header>
      <div className="summary-facts">
        <div>
          <MapPin size={17} />
          <span>
            Property address<strong>{tree.property?.address || "Not entered"}</strong>
          </span>
        </div>
        <div>
          <Building2 size={17} />
          <span>
            Property sale value
            <strong>
              {tree.property?.saleValue
                ? money.format(Number(tree.property.saleValue))
                : "No sale value entered"}
            </strong>
          </span>
        </div>
        <div>
          ...
          <span>
            Date of death
            <strong>{formattedDate(tree.succession?.dateOfDeath) || "Not entered"}</strong>
          </span>
        </div>
        <div>
          <FileText size={17} />
          <span>
            Will date
            <strong>
              {tree.succession?.basis === "intestacy"
                ? "Intestate succession"
                : formattedDate(tree.succession?.willDate) || "Not entered"}
            </strong>
          </span>
        </div>
      </div>
      <div className="summary-heirs">
        <h3>
          <UsersRound size={17} /> Heirs and published causa mortis values
        </h3>
        {heirs.length ? (
          heirs.map((heir) => {
            const item = coverageFor(heir.id);
            const fraction = approximateFraction(item?.publishedFraction || 0);
            return (
              <div className="summary-heir-row" key={heir.id}>
                <span>
                  <strong>{heir.name || "Unnamed heir"}</strong>
                  <small>{shareLabel(heir)} succession share</small>
                </span>
                <span>
                  <strong>
                    {item?.publishedCount
                  ...
                  <small>
                    {item?.publishedCount
                      ? money.format(item.publishedValue)
                      : "No declared value recorded"}
                  </small>
                </span>
              </div>
            );
          ...
          <p className="helper-text">No effective heirs have been entered.</p>
        )}
      </div>
      <div className="summary-vendor-taxes">
        <h3>
          <Calculator size={17} /> Tax payable by each living vendor
        </h3>
        {propertyTaxReports.length ? (
          propertyTaxReports.map(({ property, report }) => (
            <section className="summary-property-tax" key={property.id}>
              ...
              {report.taxSummary.vendors.length ? (
                report.taxSummary.vendors.map((vendor) => {
                    ...
                    <article className="summary-vendor-card" key={vendor.id}>
                      <div>
                        <span>
                          ...
                          <small>
                            {fraction.numerator}/{fraction.denominator} current ownership ·{" "}
                ...
                <p className="helper-text">
                  No living current vendor is recorded for this property.
                </p>
              )}

...
const ownershipLabel = (share = 0) => {
  ...
  return `${fraction.numerator}/${fraction.denominator} · ${(share * 100).toLocaleString("en-MT", { maximumFractionDigits: 4 })}%`;
};
...
export function PeopleEditor({
    ...
    () =>
      people
        .filter((person) => {
          ...
          return `${person.fullName} ${father} ${mother}`
            .toLowerCase()
  ...
  const toggleDesignation = (person, designation) => {
    ...
    const designations = existing.includes(designation)
      ? existing.filter((value) => value !== designation)
      : [...existing, designation];
    update(person.id, {
      designations,
      isDeceased:
        designation === "Deceased" ? designations.includes("Deceased") : person.isDeceased,
    });
  };
  const importGedcom = async (event) => {
    ...
    try {
      ...
      setImportStatus(
        `Imported ${result.individualCount} people and ${result.familyCount} families from ${file.name}.`,
      );
    } catch (error) {
    ...
    <section className="editor-panel">
      <header>
        ...
        <p className="helper-text">
          Import a GEDCOM file or add people manually. Parent links power the searchable index and
          relational tree.
        </p>
      </header>
      <div className="gedcom-import">
        ...
        <label>
          ...
          <select value={importMode} onChange={(event) => setImportMode(event.target.value)}>
            <option value="replace">Replace current people</option>
            <option value="merge">Merge with current people</option>
          </select>
        </label>
      </div>
      {importStatus && (
      ...
      <div className="people-index">
        <div className="index-heading">
          <div>
            ...
            <h3>People, fathers and mothers</h3>
          </div>
          <label className="people-search">
            <Search size={15} />
            <input
              aria-label="Search people index"
              value={query}
        ...
        <div className="index-table">
          ...
          {indexedPeople.map((person) => (
            <button
              type="button"
              className={`index-row ${selectedPersonId === person.id ? "selected" : ""}`}
              key={person.id}
              onClick={() => onSelectPerson?.(person.id)}
            >
              <strong>{person.fullName || "Unnamed person"}</strong>
              <span>{peopleById.get(person.fatherId)?.fullName || "—"}</span>
              <span>{peopleById.get(person.motherId)?.fullName || "—"}</span>
              <span className="index-ownership">
                {ownershipLabel(ownershipByPerson[person.id] || 0)}
              </span>
              <LocateFixed size={15} />
            </button>
          ))}
      ...
      <div className="people-list">
        {people.map((person, index) => (
          <article
            className={`person-card ${selectedPersonId === person.id ? "selected-person" : ""}`}
            key={person.id}
          >
            <div className="person-card-heading">
              <strong>{person.fullName || `Person ${index + 1}`}</strong>
              <span className="person-ownership">
                {ownershipLabel(ownershipByPerson[person.id] || 0)}
              </span>
              <button
                type="button"
            ...
            <div className="person-fields">
              <label>
                ...
                <input
                  ...
                  onChange={(event) => update(person.id, { fullName: event.target.value })}
              ...
              <label>
                ...
                <select
                  value={person.sex || ""}
                  onChange={(event) => update(person.id, { sex: event.target.value })}
                >
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(event) => update(person.id, { fatherId: event.target.value })}
                >
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(event) => update(person.id, { motherId: event.target.value })}
                >
                  <option value="">Not linked</option>
                  {people
                    .filter((item) => item.id !== person.id)
            ...
            <div className="designation-list">
              {DESIGNATIONS.map((designation) => (
                <label key={designation} className="designation-choice">
                  <input
                    type="checkbox"
                    checked={personDesignations(person).includes(designation)}
                    onChange={() => toggleDesignation(person, designation)}
                  />
                  {designation}
                </label>
              ))}
      ...
      <button
        ...
        onClick={() => onChange([...people, createPerson()])}
      >

...
export function OwnershipTransfers({ heirs, familyPeople, outsideParties, transfers, onChange }) {
  ...
  const ledger = useMemo(
    () => buildOwnershipLedger(heirs, outsideParties, transfers, familyPeople),
    [familyPeople, heirs, outsideParties, transfers],
  );
  const partyName = (id) =>
  ...
  return (
    <section className="editor-panel ownership-panel">
      <header>
        <p className="eyebrow">Title after inheritance</p>
        <h2>Ownership transfers</h2>
        <p className="helper-text">
          Record sales separately from family relationships. Each transaction updates the ownership
          ledger without changing the family tree.
        </p>
      </header>
        ...
        <div>
          ...
          <div className="owner-list">
            {ledger.owners.length ? (
              ledger.owners.map((owner) => (
                <div className="owner-row" key={owner.id}>
                  ...
                  <span>
                    ...
                    <small>
                      {["family", "family-tree"].includes(owner.source)
                        ? "Family tree"
                        : owner.type === "company"
                          ? "Outside company"
                          : "Outside individual"}
              ...
              <p className="helper-text">
                Apply or enter inherited shares to establish the first owners.
              </p>
            )}
          </div>
          <div

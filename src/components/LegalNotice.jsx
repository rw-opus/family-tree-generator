...
export const LEGAL_NOTICE_SECTIONS = [
  ...
  },
  {
    ...
    p: "The User is responsible for downloading and securely retaining independent backups of information the User considers important. To reduce loss when a page closes before a cloud save finishes, the System may temporarily keep a compact, account-and-family-and-property-and-tab-scoped record of changed initial-owner rows in browser storage on the signed-in device. It contains the owner rows, exact shares and acquisition details, plus any newly created outside-owner record needed by those rows; it is not a copy of the whole family. A tab removes its own record after the corresponding cloud save is acknowledged. A recovered, conflicting or interrupted source record may instead be retained locally and hidden after acknowledgement or dismissal so that another still-open tab's newer record is not destroyed. Permanent family deletion attempts to remove those records and leaves only a small deletion marker that prevents another open tab from recreating them; browser storage failure can prevent that local cleanup, in which case the System warns the User. Clearing the site's browser data also removes the records and marker. The User must secure that device and browser profile. To the maximum extent permitted by law, the operator accepts no liability for loss, corruption, deletion, unauthorised alteration or inaccessibility of data, howsoever caused.",
  },
  {

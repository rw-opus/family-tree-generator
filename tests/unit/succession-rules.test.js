...
describe("section-specific historical-law warnings", () => {
  it("warns only when the death predates a verified change to an applicable section", () => {
    const review = legacyHistoricalLawReview("1990-04-02", ["808", "809", "825"]);

    expect(review.required).toBe(true);
    expect(review.articles).toEqual(["825"]);
    expect(review.warning).toContain("article 825");
  ...
  it("recognises only the dedicated historical-law warning prefix", () => {
    expect(isLegacyHistoricalLawWarning(legacyHistoricalLawWarning("1990-04-02", ["825"]))).toBe(
      true,
    );
